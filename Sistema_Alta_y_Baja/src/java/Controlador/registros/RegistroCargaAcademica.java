 ...
 * @author trdni
 */
public class RegistroCargaAcademica extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
      ...
      String exp=request.getParameter("TEE");
       ...
       ConsultaCargaAcademica co2 = new ConsultaCargaAcademica();
       ConsultaCargaAcademica com = new ConsultaCargaAcademica();
       ConsultaCargaAcademica comi = new ConsultaCargaAcademica();
       ConsultaCargaAcademica coj = new ConsultaCargaAcademica();
       ConsultaCargaAcademica cov = new ConsultaCargaAcademica();
       int a=0;
       int b=0;
       int c=0;
       int d=0;
       ...
       if(snl.equals("No")){
           ...
           b=com.ValidarTraslapeM(MIH, MFH,Martes,Periodo);
       } if(snmi.equals("No")){
        
       }else{
          c=comi.ValidarTraslapeMI(MIIH, MIFH,Miercoles,Periodo);  
       } if(snj.equals("No")){
        
       }else{
          d=coj.ValidarTraslapeJ(JIH, JFH,Jueves,Periodo);
       } if(snv.equals("No")){
        
       }else{
           e=cov.ValidarTraslapeV(VIH, VFH,Viernes,Periodo);
       }
           ...
           if(b==0)
           {
               if(c==0)
               {
                   if(d==0)
                   {
                       if(e==0)
                       {    
                           if(co2.registrar_carga(Periodo,cred, nrc, CATE, hsm ,progc, academia, seccion, LIH,LFH, MIH, MFH,MIIH,MIFH,JIH,JFH,VIH,VFH,Lunes,Martes,Miercoles,Jueves,Viernes,Cupo,CATED,0,snl,snm,snmi,snj,snv,depa))
                           {
                               response.getWriter().println("Exito al guardar");            
                           }else
                           {
                               response.getWriter().println("Error al guardar"+Periodo+cred+ nrc+ CATE+ hsm +progc+ academia +seccion+ LIH+LFH+ MIH+ MFH+MIIH+MIFH+JIH+JFH+VIH+VFH+Lunes+Martes+Miercoles+Jueves+Viernes+Cupo+CATED+0+snl+snm+snmi+snj+snv);
                           }
