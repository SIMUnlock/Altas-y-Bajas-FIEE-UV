...
public class Modificarpassword extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
       ...
       int id=Integer.parseInt(request.getParameter("n"));
        String pass=request.getParameter("nc");
        String pass1=request.getParameter("ca");
        
        ConsultaUsuario co = new ConsultaUsuario();
        ConsultaUsuario con=new ConsultaUsuario();
        Usuario u= co.getUsuarios(id);
        if(u.getContrasena().equals(pass1))
        {
        if(con.modificar_password(id,pass)){
