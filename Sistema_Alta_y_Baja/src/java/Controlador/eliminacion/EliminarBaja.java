 ...
 * @author trdni
 */
public class EliminarBaja extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
      ...
      ConsultarBaja a= new ConsultarBaja();
      ConsultarBaja ba= new ConsultarBaja();
      Baja bajas=ba.getBaja(id);
      
       ConsultaCargaAcademica carg = new ConsultaCargaAcademica();
      ConsultaCargaAcademica carga = new ConsultaCargaAcademica();
      
      CargaAcademica cup=carga.getCargaNrc(bajas.getNRC());
      
      int cupo=Integer.parseInt(cup.getCupo());
      int cupoN = cupo+1;
      
      
        if(a.eliminar_Baja(id)){
            carg.Dar_AltaNRC(bajas.getNRC(),cupoN);
           response.getWriter().println("Baja eliminada");
        }else{
